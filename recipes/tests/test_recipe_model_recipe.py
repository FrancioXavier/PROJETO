...
class RecipeModelTest(RecipeTestBase):
    ...
    def test_recipe_html_is_false_by_default(self):
        recipe = self.make_recipe_no_default()
        self.assertFalse(
            recipe.preparation_steps_is_html,
            msg='testa se o campo está False'
        )
    ...
    def test_recipe_string_representation(self):
        self.recipe.title = 'String Representation'
        self.recipe.full_clean()
        self.recipe.save()
        self.assertEqual(
            str(self.recipe),
            'String Representation',
            msg=f'recipe string representation must be "{self.recipe.title}"'
        )
